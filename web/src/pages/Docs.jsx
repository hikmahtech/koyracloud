            ...
            <div className="card overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <tbody>
                  ...
                  <Field name="runtime" req><span className="mono">python</span>, <span className="mono">node</span>, <span className="mono">python+node</span>, <span className="mono">static</span>, <span className="mono">dockerfile</span>, or <span className="mono">go</span> (two-stage <span className="mono">golang:1.23</span> → distroless build; no shell or python3 in the runner, so <span className="mono">healthcheck</span>/<span className="mono">predeploy</span> aren't supported — koyracloud rejects the combination).</Field>
                  ...
                  <Field name="root">Build-context subdirectory for a monorepo app — e.g. <span className="mono">marketing/site</span>. The Docker context (and <span className="mono">dockerfile</span> path) are scoped to it; the rest of the repo can build separately. Blank = repo root.</Field>
                  ...
                  <Field name="spa">For <span className="mono">runtime: static</span>: <span className="mono">true</span> always falls back to <span className="mono">index.html</span> (SPA); <span className="mono">false</span> serves real 404s. Omit to auto-detect — a <span className="mono">404.html</span> in the build (Next.js exports ship one) → real 404s, else SPA fallback.</Field>
                  <Field name="headers">For <span className="mono">runtime: static</span>: extra response headers (string map) sent on every response. <span className="mono">X-Content-Type-Options: nosniff</span> and <span className="mono">X-Frame-Options: SAMEORIGIN</span> are sent by default unless overridden here.</Field>
